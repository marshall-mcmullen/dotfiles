...
if [[ ! -v SSH_AGENT_PID ]] || ! kill -0 "${SSH_AGENT_PID}" &>/dev/null; then
  ...
  echo "export SSH_AGENT_PID=${SSH_AGENT_PID}" >> ~/.ssh/agent.env
    ...
    echo -e "${COLOR_BOLD}${COLOR_GREEN}>>${COLOR_NONE} Found existing ssh-agent: ${COLOR_CYAN}${SSH_AGENT_PID}${COLOR_NONE}"
fi
...
for key in ~/.ssh/id_*; do
    ...
    if ! ssh-add -l | grep -q "$(ssh-keygen -lf "${key}" | awk '{print $2}')"; then
        echo -e "${COLOR_BOLD}${COLOR_GREEN}>>${COLOR_NONE} Adding SSH key: ${COLOR_CYAN}${key}${COLOR_NONE}"
        ssh-add "${key}"
    else
        echo -e "${COLOR_BOLD}${COLOR_GREEN}>>${COLOR_NONE} Known ssh key: ${COLOR_CYAN}${key}${COLOR_NONE}"
    fi
done
